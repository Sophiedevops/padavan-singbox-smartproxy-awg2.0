...
2.     Выполните команду   ```bash /opt/awg2_singbox/build.sh   ```  для пересборки **run.json** с новыми параметрами.
3.     Перезапустите сервив командой ```bash /opt/awg2_singbox/sing-box run -c /opt/awg2_singbox/run.json > /dev/null 2>&1 & ```
> [!IMPORTANT]
