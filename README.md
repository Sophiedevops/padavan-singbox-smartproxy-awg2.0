...
<br>
Трафик РФ идет напрямую,
   Локальный резолв: Для российских доменов (geoip-ru) используется yandex-udp (77.88.8.8). Это гарантирует, что Кинопоиск, Госуслуги и банки будут открываться максимально быстро и не заблокируют вас из-за «заграничного» IP.
   весь остальной мир — через VPN.
</details>
...
1. 📤 Загрузите ваши файлы `.conf` от AmneziaWG в папку `/opt/awg2_singbox/configs` на роутере, удалив неиспользуемые/нерабочие.

2. Выполните команду для пересборки **run.json** с новыми параметрами:
```bash
   /opt/awg2_singbox/build.sh
```

3. Перезапустите сервис командой:
```bash
   /opt/awg2_singbox/sing-box run -c /opt/awg2_singbox/run.json > /dev/null 2>&1 &
```
